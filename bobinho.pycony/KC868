...
class KC868_Hx:
    ...
    def __init__(self, controller_ip, controller_port=None):
        ...
        if not self._is_supported(controller_type):
            raise ControllerNotSupportedError(controller_type)
...
class ControllerNotSupportedError(TypeError):
    def __init__(self, found_controller_type):
        super().__init__(f"Controller found ({found_controller_type}) is not supported")